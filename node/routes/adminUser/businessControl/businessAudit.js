...
router.get('/businessAudit', verToken, limiter(60, 5), async(req, res) => {
    try {
        ...
        const totalResultsBody = `SELECT COUNT(*) AS total FROM business WHERE status = 1`;
        ...
        // 在数据库中查询待审核商家信息
        const result = await sqlConn('SELECT * FROM business WHERE status = 1 LIMIT ? OFFSET ?', [limit, offset]);
