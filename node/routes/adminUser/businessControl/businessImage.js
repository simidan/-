...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        cb(null, `business-${file.fieldname}-${Date.now()}${path.extname(file.originalname)}`);
    }
});
...
router.post('/avatarUpdate/:id', (req, res, next) => {
    ...
    try {
        // 在数据库中更新用户头像的文件路径
        const result = await sqlConn('UPDATE business SET image = ? WHERE business_id = ?', [filePath, businessId]);
