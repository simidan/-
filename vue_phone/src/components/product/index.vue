      ...
      <van-col span="24">
        <div class="splite">
          <van-dropdown-menu class="menu">
            <van-dropdown-item v-model="SearchInfoOther.order" :options="sortByOptions"/>
            <van-field v-model="SearchInfoOther.origin" label="产地" class="item" @click="defaultAddressPopup"/>
...
import productSearchOtherControl from '@/store/product/productSearchOther'
import messageBox from '@/utils/messageBox';
import { debounce } from '@/utils/debounce';
import router from '@/router';

const locationStore = locationControl()
const productSearchOtherStore = productSearchOtherControl()

const route = useRoute()

const SearchInfoOther = ref({
    category_id: 0,
    sub_category_id: 0,
    limit: 10,
    page: 1,
...
const harvestDateStartconfirm = ()=>{
  ...
  SearchInfoOther.value.harvestDateStart =  harvestDateStart.value.join('-')
}
const harvestDateStartcancel = ()=>{
  harvestDateStartTarget.value = false
  SearchInfoOther.value.harvestDateStart =  ''
}

const harvestDateEndconfirm = ()=>{
  harvestDateEndTarget.value = false
  SearchInfoOther.value.harvestDateEnd =  harvestDateEnd.value.join('-')
  harvestDate.value = SearchInfoOther.value.harvestDateStart + ' - ' + SearchInfoOther.value.harvestDateEnd
  productList.value = []
  getProcuct()
}
const harvestDateEndcancel = ()=>{
  harvestDateEndTarget.value = false
  SearchInfoOther.value.harvestDateEnd =  ''
}
...
const defaultAddressConfirm =(value: any)=>{
  SearchInfoOther.value.origin = value.selectedOptions[0].text+'-'+value.selectedOptions[1].text+'-'+value.selectedOptions[2].text
  productList.value = []
  getProcuct()
...
const handleScroll = (event: Event) => {
  ...
  if (scrollHeight - (scrollTop + offsetHeight) < 100) {
    ...
    if (SearchInfoOther.value.page < totalPages.value) {
        SearchInfoOther.value.page += 1;
        getProcuct()
      }
  }
};
...
const getProcuct = async()=>{
  try{
      const res = await productSearchOtherStore.productSearchOther(SearchInfoOther.value)
      if(res.statusCode === 200){
        if(res.data){
          ...
          if(SearchInfoOther.value.page === 1){
...
watch(() => SearchInfoOther.value.order, ()=>{
  SearchInfoOther.value.sortBy = 'price'
  productList.value = []
  getProcuct()
});
...
onMounted(async()=>{
  if(route.query.category_id && route.query.sub_category_id){
    SearchInfoOther.value.category_id = Number(route.query.category_id)
    SearchInfoOther.value.sub_category_id = Number(route.query.sub_category_id)
    getProcuct()
  }
})
</script>
