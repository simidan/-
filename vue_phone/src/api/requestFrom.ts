...
const requestFrom = axios.create({
    ...
    headers: { 'Content-Type': 'multipart/form-data' } // 设置默认请求头
});

// 请求拦截器
requestFrom.interceptors.request.use((config) => {
...
// 响应拦截器
requestFrom.interceptors.response.use((response: AxiosResponse) => {
    // 对响应数据进行处理
    return response;
}, (error: AxiosError) => {
    // 响应出错时的处理
...
export default requestFrom; // 导出 axios 实例
